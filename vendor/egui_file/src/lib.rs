...
use egui::{Align2, Button, Context, Id, Key, Layout, Pos2, RichText, ScrollArea, TextEdit, Ui, Vec2, Vec2b, Window};
use fs::FileInfo;
use fs::Fs;
...
impl FileDialog {
  ...
  pub fn select_folder() -> Self {
    FileDialog::new(DialogType::SelectFolder)
  }

  /// Create dialog that prompts the user to open a file.
  pub fn open_file() -> Self {
    FileDialog::new(DialogType::OpenFile)
  }

  /// Create dialog that prompts the user to save a file.
  pub fn save_file() -> Self {
    FileDialog::new(DialogType::SaveFile)
  }

  /// Constructs new file dialog.
  /// - `dialog_type`: Type of file dialog to create. See [DialogType].
  fn new(dialog_type: DialogType) -> Self {
    let path = std::env::current_dir().unwrap_or_default();
    let filename_edit = String::new();
  ...
  /// Set the initial path. Default is `env::current_dir`.
  ...
  /// Set the window ID. See [egui::Window::id].
  pub fn id(mut self, id: impl Into<Id>) -> Self {
    self.id = Some(id.into());
    self
  }

  /// Set the window anchor. See [egui::Window::anchor].
  pub fn anchor(mut self, align: Align2, offset: impl Into<Vec2>) -> Self {
    self.anchor = Some((align, offset.into()));
    self
  }

  /// Set the window's current position. See [egui::Window::current_pos]
  pub fn current_pos(mut self, current_pos: impl Into<Pos2>) -> Self {
    self.current_pos = Some(current_pos.into());
    self
  }

  /// Set the window's default position. See [egui::Window::default_pos].
  pub fn default_pos(mut self, default_pos: impl Into<Pos2>) -> Self {
    self.default_pos = Some(default_pos.into());
    self
  }

  /// Set the window's default size. See [egui::Window::default_size].
  pub fn default_size(mut self, default_size: impl Into<Vec2>) -> Self {
    self.default_size = default_size.into();
    self
  }

  /// Enable/disable resizing the window.  See [egui::Window::resizable].
    ...
    egui::TopBottomPanel::top("egui_file_top").show_inside(ui, |ui| {
      ui.horizontal(|ui| {
        ui.add_enabled_ui(self.path.parent().is_some(), |ui| {
          let response = ui
            .add_sized([23.0, 20.0], Button::new("⬆"))
