...
impl QuadRatPer4
{
    ...
    fn fixed_cycle_points(c: &Cplx) -> ComplexVec
    {
        let c_minus_1 = c - 1.;
        let c_inv = c_minus_1.inv();
        let c_sq = c.powi(2);
        let cubic_scale = c_inv * (c_minus_1 + c_sq);
        let linear_scale = c_inv * (c - (c_sq + c_sq));
        let depressed_quad = 1. - 3. * cubic_scale;
        let discriminant_scale = 9. * cubic_scale + 27. * linear_scale - 2.;
        let discriminant = (-4. * depressed_quad.powf(3.) + discriminant_scale.powi(2)).sqrt();
        let cubic_root = (0.5 * (discriminant_scale + discriminant)).powf(ONE_THIRD);
        let cubic_term = cubic_root / 3.;
        let inverse_term = depressed_quad / (3. * cubic_root);
        let root_a = -cubic_term * OMEGA_BAR - inverse_term * OMEGA + ONE_THIRD;
        let root_b = -cubic_term * OMEGA - inverse_term * OMEGA_BAR + ONE_THIRD;

        vec![-cubic_term - inverse_term + ONE_THIRD, root_a, root_b]
    }

    fn period_2_cycle_points(c: &Cplx) -> ComplexVec
    {
        let c_sq = c.powi(2);
        let numerator_scale = 3. * c_sq;
        let denominator = 0.5 / (c - 1.);
        let discriminant = (numerator_scale.powi(2) - c * (8. * c_sq - 6. * c + 4.) + 1.).sqrt();

        vec![
            denominator * (numerator_scale + discriminant - 1.),
            denominator * (numerator_scale - discriminant - 1.),
        ]
    }

    fn period_3_cycle_points(c: &Cplx) -> ComplexVec
    {
        let c_sq = c.powi(2);
        let coeffs = [
            c_sq * c * horner!(c, 1., -7., 18., -20., 8.),
            c_sq * horner!(c, -4., 25., -54., 41., 4., -12.),
    ...
    fn period_4_cycle_points(c: &Cplx) -> ComplexVec
    {
        let c_sq = c.powi(2);
        let c_cu = c * c_sq;
        let c_four = c_sq.powi(2);
        let coeffs = [
            c_cu * c_four * horner!(c, -1., 12., -61., 170., -280., 272., -144., 32.),
            c_four
                * horner!(
                    c, 1., -15., 103., -419., 1089., -1817., 1835., -896., -72., 272., -80.
                ),
            c_cu * horner!(
                c, -4., 57., -360., 1300., -2868., 3747., -2293., -527., 1686., -732., -104., 96.
            ),
            c_sq * horner!(
                c, 6., -79., 445., -1345., 2127., -841., -3011., 5721., -3916., 382., 726., -144.,
                -72.
            ),
            c * horner!(
                c, -4., 45., -191., 261., 737., -3856., 7348., -6869., 2028., 1633., -1223., -90.,
                151., 34.
            ),
            horner!(
                c, 1., -6., -21., 322., -1375., 2999., -3272., 469., 3191., -3641., 1294., 192.,
                -105., -41., -9.
            ),
            horner_monic!(
                c, -2., 24., -117., 264., -90., -1028., 2817., -3546., 2169., -238., -392., 115.,
                26., -3.
            ),
            horner!(
                c, 1., -14., 87., -312., 701., -987., 774., -121., -362., 329., -98., 1., -1., 2.
            ),
            c_sq * c_cu * horner_monic!(c, -1., 7., -21., 35., -35., 21., -7.),
        ];
        let mut roots = solve_polynomial(coeffs);
    ...
    fn marked_cycle_curve_period_3(self) -> CoveringMap<Self>
    {
        ...
        CoveringMap::new(self, param_map as fn(Cplx) -> (Cplx, Cplx)).with_orig_bounds(bounds)
    }
}
...
impl MarkedPoints for QuadRatPer4
{
    ...
    {
        match period {
            1 => Self::fixed_cycle_points(c),
            2 => Self::period_2_cycle_points(c),
            3 => Self::period_3_cycle_points(c),
            4 => Self::period_4_cycle_points(c),
            _ => vec![],
        }
    }
}
...
impl HasDynamicalCovers for QuadRatPer4
{
    ...
    {
        match period {
            3 => self.marked_cycle_curve_period_3(),
