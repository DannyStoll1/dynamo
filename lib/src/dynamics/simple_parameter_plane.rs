use super::{julia::JuliaSet, orbit::*};
use crate::{
...
pub trait SimpleParameterPlane: Sync + Send + DynClone
{
    ...
    fn early_bailout(&self, _start: ComplexNum, _param: ComplexNum) -> EscapeState<ComplexNum>
    {
        EscapeState::NotYetEscaped
    }

    fn stop_condition(&self, iter: Period, z: ComplexNum) -> EscapeState<ComplexNum>
    {
    ...
    fn encode_escape_result(
        &self,
        state: EscapeState<ComplexNum>,
        base_param: ComplexNum,
    ) -> PointInfo<ComplexNum>
    {
        match state
        {
    ...
    ) -> EscapeState<ComplexNum>
    {
        ...
        let orbit = SimpleOrbit::new(|z, c| self.map(z, c), stop_condition, start, param);
    ...
    fn get_orbit_and_info(
        &self,
        param: ComplexNum,
    ) -> (ComplexVec, OrbitInfo<ComplexNum, ComplexNum>)
    {
        let start = self.start_point(param);
        let orbit = CycleDetectedOrbitFloyd::new(
            |z, c| self.map(z, c),
